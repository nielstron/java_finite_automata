	...
	public FiniteAutomaton<Set<S>, T> constructDNF(FiniteAutomaton<S, T> fa) {
		...
		while (!work.isEmpty()) {
			...
			for (T input : fa.getInputValues()) {
				...
				// Insert that into the new Transitionfunction => set of size
				// one as this new automaton is deterministic
		...
		return new FiniteAutomaton<Set<S>, T>(discovered, inputs, transitionFunc,
				new HashSet<Set<S>>(Arrays.asList(initState)), accepting);
	}
